    ...
    <div class={`max-w-4xl mx-auto transition-all duration-700 delay-200 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
      <div class="mb-8">
        <div class="bg-gray-800 rounded-lg p-6 mb-8">
          ...
          <div class="bg-gray-900 rounded p-4 font-mono text-sm mb-4">
            <p class="text-gray-400 mb-2"># Install Rust and Cargo (required for building cassettes)</p>
            <p class="text-green-400">curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh</p>
            <p class="text-green-400">source $HOME/.cargo/env</p>
            <p class="text-green-400">rustup target add wasm32-unknown-unknown</p>
          </div>
          <p class="text-gray-300 text-sm">
            Cassette requires Rust to compile events into WebAssembly modules. The WASM target is needed for cassette generation.
          </p>
        </div>
