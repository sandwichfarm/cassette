...
bindings_path = str(Path(__file__).parent.parent.parent / "bindings" / "py")
...
def benchmark_cassette(cassette_path: str, iterations: int = 100, debug: bool = False) -> Dict[str, Any]:
    ...
    # Get cassette info
    cassette_name = os.path.basename(cassette_path)
    ...
    try:
        ...
        if isinstance(count_response, str):
            ...
            if parsed[0] == "COUNT" and len(parsed) > 2:
                event_count = parsed[2].get("count", 0)
            else:
                event_count = 0
        else:
            event_count = 0
    ...
    print(f"ℹ️  Cassette: {cassette_name}")
    ...
    results = {
        ...
        "info": {
            "name": cassette_name,
            "description": "Benchmark cassette",
            "version": "1.0.0"
        }
    }
...
def main():
    ...
    parser.add_argument("--iterations", "-i", type=int, default=None,
                       help="Iterations per test (overrides config)")
    ...
    # Load config if iterations not specified
    config = None
    get_iterations_for_cassette = None
    if args.iterations is None:
        try:
            sys.path.insert(0, str(Path(__file__).parent.parent / "scripts"))
            from load_config import load_config, get_iterations_for_cassette
            config = load_config()
        except Exception as e:
    ...
    for cassette_path in args.cassettes:
        ...
        elif config is not None and get_iterations_for_cassette is not None:
            iterations = get_iterations_for_cassette(cassette_path, config, 'python')
        else:
            iterations = 100  # Default fallback
        
        try:
            result = benchmark_cassette(cassette_path, iterations, args.debug)
