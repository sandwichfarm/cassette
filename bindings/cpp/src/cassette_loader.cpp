    ...
    if (result.find('\n') != std::string::npos) {
        ...
        while (std::getline(stream, message)) {
            ...
            try {
                ...
                if (msg_type != "NOTICE" && msg_type != "EVENT" && msg_type != "EOSE" && msg_type != "OK" && msg_type != "CLOSED") {
                    if (debug) {
...
SendResult Cassette::send(const std::string& message) {
    ...
    try {
        ...
        if (msg_data.is_array() && msg_data.size() >= 2) {
            ...
            if (msg_type == "REQ") {
                ...
                is_req_message = true;
                subscription_id = msg_data[1].get<std::string>();
    ...
    if (is_req_message) {
        return collect_all_events_for_req(message, subscription_id);
    }
...
std::vector<std::string> Cassette::collect_all_events_for_req(const std::string& message, const std::string& subscription_id) {
    if (debug) {
        std::cerr << "[Cassette] Collecting all events for REQ subscription: " << subscription_id << "\n";
    }
    ...
    while (true) {
        std::string response = send_single(message);
        
        // Empty response means no more events
        if (response.empty()) {
            if (debug) {
                std::cerr << "[Cassette] Received empty response, stopping\n";
            }
            break;
        }
        ...
        try {
            ...
            if (parsed.is_array() && parsed.size() >= 1) {
                ...
                if (msg_type == "EOSE") {
                    if (debug) {
                        std::cerr << "[Cassette] Received EOSE for subscription " << subscription_id << "\n";
                    }
                    results.push_back(response);
                    break;
                } else if (msg_type == "CLOSED") {
                    if (debug) {
                        std::cerr << "[Cassette] Received CLOSED for subscription " << subscription_id << "\n";
                    }
                    results.push_back(response);
                    break;
                }
            ...
            if (debug) {
                std::cerr << "[Cassette] Failed to parse response, stopping\n";
            }
            break;
        }
    ...
    for (const auto& r : results) {
        try {
            ...
            if (parsed.is_array() && parsed.size() >= 1 && parsed[0] == "EOSE") {
                has_eose = true;
                break;
            }
        } catch (...) {}
    ...
    // If no EOSE, add one
    if (!has_eose) {
        json eose = json::array({"EOSE", subscription_id});
        results.push_back(eose.dump());
    }
